...
namespace nts
{
    Component4081::Component4081(const std::string &name) : AGate(name, 14)
    {
    ...
    void Component4081::setLink(std::size_t pin, std::shared_ptr<IComponent> other, std::size_t otherPin)
    {
        (void)otherPin;
    ...
    void Component4081::simulate(std::size_t tick)
    {
        ...
        for (size_t i = 0; i < 4; i++) {
            ...
            Tristate output = ANDComponent::calculateState(input1, input2);
    ...
    Tristate Component4081::compute(std::size_t tick)
    {
        this->simulate(tick);
        return UNDEFINED;
    }
